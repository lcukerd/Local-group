package com.lcukerd.nearbytest;
...
public abstract class ConnectionsActivity extends AppCompatActivity
    ...
    private CountDownTimer timer = new CountDownTimer(120000, 1000) {
        ...
        {
            float speed = (done-lastsent)/ 1000000;
                ...
                public void onPayloadTransferUpdate(String endpointId, final PayloadTransferUpdate update)
                {
                    ...
                    switch (update.getStatus()) {
                        case PayloadTransferUpdate.Status.IN_PROGRESS:
                            ...
                            done = update.getBytesTransferred();
                            if (notstarted)
                            {
                                notstarted = false;
                                timer.start();
    ...
    {
        Uri uri = Uri.parse("content://com.android.providers.media.documents/document/video%3A30269");
        ...
        try {
            ...
            Nearby.Connections.sendPayload(mGoogleApiClient,
                    new ArrayList<>(mEstablishedConnections.keySet()),
                    Payload.fromBytes(payloadFilenameMessage.getBytes("UTF-8")));

            // Finally, send the file payload.
            Nearby.Connections.sendPayload(mGoogleApiClient, new ArrayList<>(mEstablishedConnections.keySet()), filePayload);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
